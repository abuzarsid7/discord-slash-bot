...
## 6. Multi-Server Isolation (Guild Scoping)
To support multi-server deployments where multiple Discord guilds (servers) connect to the same bot instance without sharing rules or leaking interaction logs, we implemented strict guild isolation:
* **Database Strategy:** In our Prisma schema (`prisma/schema.prisma`), both `command_log` and `config` models include a required `guildId` column (with `@default("default")` to ensure backward compatibility and zero-downtime migrations). For configuration records, we replaced the single unique index on `key` with a compound unique index `@@unique([guildId, key], name: "guildId_key")`, enabling each server to define its own isolated trigger keywords (`flagged_keywords`) and destination channels (`mirror_webhook_url`).
